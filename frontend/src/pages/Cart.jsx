...
const Cart = () => {
    ...
    const {
        products, currency, cartItems, removeFromCart, updateCartItem,
        navigate, getCartAmount, getCartCount, axios, user, setCartItems
    } = useAppContext()
    ...
    const getUserAddress = async() => {
        try {
            const {data} = await axios('/api/address/get')
                ...
                {cartArray.map((product, index) => (
                    <div key={index} className="grid grid-cols-[2fr_1fr_1fr] text-gray-500 items-center text-sm md:text-base font-medium pt-3">
                        <div className="flex items-center md:gap-6 gap-3">
                            <div onClick={() => {
                                navigate(`products/${product.category.toLowerCase()}/${product}`);
                                scrollTo(0,0)
                            }} 
